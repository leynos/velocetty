/** @file Ensures `bun audit` only fails for advisories covered by the
...
function getLedgerExpiryError(entry, advisoryId, referenceDateValue) {
  ...
  const expiryBoundary = dateOnlyPattern.test(rawExpiry) ? expiryDate.getTime() + DAY_MS : expiryDate.getTime();
...
function reportAllowedAdvisories(expected) {
  ...
  console.info(`All reported advisories are covered by the audit exception ledger (${advisoryCount} ${suffix}).`);
  return 0;
}
 ...
 *   result containing advisories and the bun exit status.
...
export function evaluateAudit(payload, options = {}) {
  ...
  const {ledgerByAdvisory, allowedIds} = buildLedgerMaps(workspaceKeys, auditExceptions, referenceDate);
