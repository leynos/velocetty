...
- In esbuild, keep renderer loaders split between
  `'.module.css': 'local-css'` and `'.css': 'css'`, and ensure `plugins`
  compose PostCSS transforms before CSS module scoping so shared token imports
  remain stable.
