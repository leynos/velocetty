/** @file Verifies split-pane drag maths stays anchored to the drag-start sizes. */
...
test('applies drag deltas from the drag-start snapshot instead of compounding them', async () => {
  ...
  try {
    ...
    await act(async () => {
      divider?.dispatchEvent(new window.MouseEvent('mousedown', {bubbles: true, clientX: 300, clientY: 0}));
      await waitFor(0);
    });

    await act(async () => {
      window.dispatchEvent(new window.MouseEvent('mousemove', {bubbles: true, clientX: 350, clientY: 0}));
      await waitFor(0);
    });
    await act(async () => {
      window.dispatchEvent(new window.MouseEvent('mousemove', {bubbles: true, clientX: 360, clientY: 0}));
      await waitFor(0);
    });
