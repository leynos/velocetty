 ...
 * - each test runs in an isolated temporary root that it tears down itself
 *
 * Usage:
 * - run this module directly with
 *   `bun test test/unit/ensure-directory-path.test.ts`
 * - the suite manages its own temporary directories per test, so no manual
 *   fixture cleanup is required
 *
 * Related modules:
...
const withTemporaryRoot = async (runTest: (temporaryRoot: string) => Promise<void>) => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'ensure-directory-path-'));
  try {
    await runTest(temporaryRoot);
  } finally {
